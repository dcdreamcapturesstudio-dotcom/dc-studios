    ...
    <header className="fixed top-0 w-full z-50">
      <div className="md:px-10 px-4 h-20 flex items-center justify-between">
        <div className=" px-1 py-1 rounded">
          <Link href="/" className={`text-3xl tracking-wider font-serif transition-colors duration-300 ${isScrolled ? 'text-black' : 'text-white'}`}>
            <span className="font-sedgwick">DC Studios</span>
          </Link>
        </div>
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-neutral-600 bg-white border border-neutral-200 px-3 py-1 rounded">
          ...
          <div className="relative group flex items-center h-full">
            <Link href="/#gallery" className="hover:text-black transition-colors flex items-center gap-1 py-1">
              Gallery
            </Link>
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300">
              <div className="bg-white/90 backdrop-blur-md border border-neutral-200 rounded-lg p-2 min-w-48 shadow-2xl flex flex-col gap-1">
                {categories.map((category, idx) => (
                  <Link 
                    key={idx} 
                    href={`/gallery/${category.slug}`}
                    className="px-4 py-2 hover:bg-neutral-100 hover:text-black rounded transition-colors text-neutral-600 text-sm whitespace-nowrap"
                  >
                    {category.name}
                  </Link>
                ))}
      ...
      >
        <nav className="flex flex-col gap-4 text-xl text-neutral-300">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 pl-4 border-l border-neutral-700">
              {categories.map((category, idx) => (
                <Link 
                  key={idx} 
                  href={`/gallery/${category.slug}`}
